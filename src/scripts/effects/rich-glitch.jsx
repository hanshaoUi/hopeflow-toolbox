...
with (win.add('panel')) {
  ...
  with (add('group')) {
    ...
    add('statictext', undefined, '方向:');
    var __container = add('dropdownlist', undefined, '水平,垂直'.split(','));
  ...
  with (add('group')) {
    ...
    var preview = add('checkbox', undefined, '预览');
  }
}
...
with (win.add('group')) {
  ...
  var cancelBtn = add('button', undefined, '取消'),
    copyButton = add('button', undefined, '应用副本'),
    applyBtn = add('button', undefined, '确定');
...
if ($.hopeflow) {
  ...
  __fragments.text = args.fragments || 10;
  __offsetX.text = args.offsetX || 5;
  __offsetY.text = args.offsetY || 0;
  __levels.text = args.levels || 0;

  // Map direction to dropdown index or text simulation
  var dir = args.direction || 'horizontal';
  __container.selection = (dir === 'vertical') ? 1 : 0;
