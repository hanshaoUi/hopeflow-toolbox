...
function runWith() {
	...
	var noDocs = "请打开包含" + contents + "的文档";
	var selectProb = "请选择" + contents;
	//========Handle Situation============
	try {
...
function main() {
	...
	if (versionNum >= 13) {
		var dlg = new Window('dialog', '弧形扭曲器', [100, 100, 460, 260]);
		dlg.radioPnl = dlg.add('panel', [15, 15, 100, 150], '指仓类型');
		...
		dlg.radioPnl.leftHandRb = dlg.radioPnl.add('radiobutton', [5, 25, 95, 45], '左侧');
		dlg.radioPnl.rightHandRb = dlg.radioPnl.add('radiobutton', [5, 45, 95, 65], '右侧');
		dlg.radioPnl.mirrorHandRb = dlg.radioPnl.add('radiobutton', [5, 65, 95, 85], '镜像');
		dlg.radioPnl.fixedHandRb = dlg.radioPnl.add('radiobutton', [5, 85, 95, 105], '固定');
		dlg.radioPnl.smoothHandRb = dlg.radioPnl.add('radiobutton', [5, 105, 95, 125], '平滑');
		dlg.radioPnl.mirrorHandRb.value = true;

		//							-------------Slider Panel-------------------

		dlg.sliderPnl.add('statictext', [15, 5, 200, 25], '改变指仓长度 (像素)');
		...
		dlg.sliderPnl.add('statictext', [15, 55, 200, 75], '改变指仓角度 (度)');
		...
		dlg.btnPnl.buildBtn = dlg.btnPnl.add('button', [100, 3, 145, 27], '完成', { name: 'ok' });
		...
		// pre CS2
		var mod = 0;
		modUI = prompt('(左)仅左侧, (右)仅右侧, (固定)固定角度,(M)镜像角度 或 (S)平滑180度:', mod).toUpperCase();
		chUI = prompt('改变角度,距离:', achange + ',' + dchange);
...
function processUI() {
	...
	if (changed == 0) {
		alert('未有任何改变。未选择路径页图，或路径页图无可以改变的文字。');
	}
}
