...
function main() {
  ...
  if (args.mode === 'exportSelection') return exportSelection();
  if (args.mode === 'createContour') return createContour(args);
  return $.hopeflow.utils.returnError('未知的图片外轮廓模式');
}
...
function exportSelection() {
  ...
  if (!app.selection.length || app.selection.typename === 'TextRange') {
    return $.hopeflow.utils.returnError('请先选择图片或对象');
  }
  ...
  var tempDoc = app.documents.add(DocumentColorSpace.RGB, widthPt, heightPt);
  tempDoc.rulerOrigin = [0, 0];

  var bg = tempDoc.layers[0].pathItems.rectangle(heightPt, 0, widthPt, heightPt);
  bg.filled = true;
  bg.stroked = false;
  bg.fillColor = rgbColor(255, 255, 255);
  try { bg.zOrder(ZOrderMethod.SENDTOBACK); } catch (errBgOrder) {}
  ...
  for (var s = 0; s < sourceSelection.length; s++) {
    ...
    try { dup.zOrder(ZOrderMethod.BRINGTOFRONT); } catch (errDupOrder) {}
  }

  var tempFile = new File(Folder.temp.fsName + '/hopeflow_image_contour_' + (new Date().getTime()) + '.jpg');
  ...
  tempDoc.exportFile(tempFile, ExportType.JPEG, options);
  ...
  return $.hopeflow.utils.returnResult({
    ...
    bounds: padded,
    itemType: sourceSelection.length === 1 ? sourceSelection[0].typename : 'Selection',
    itemName: sourceSelection.length === 1 ? (sourceSelection[0].name || '') : sourceSelection.length + ' items'
  });
}
...
function createContour(args) {
  if (!app.documents.length) return $.hopeflow.utils.returnError('没有打开的文档');
  if (!app.selection.length || app.selection.typename === 'TextRange') {
    return $.hopeflow.utils.returnError('创建轮廓前请先选择原始对象');
  }

  var points = args.points || [];
  if (points.length < 3) return $.hopeflow.utils.returnError('轮廓点不足');

  var doc = app.activeDocument;
  var origItem = app.selection[0];
  ...
  var strokeValue = enableStroke ? convertUnitValue(Math.abs(num(args.stroke, 0.25)), String(args.strokeUnit || 'pt'), 'pt') : 0;
  ...
  app.selection = [path];
  return $.hopeflow.utils.returnResult({ message: '图片外轮廓已创建', count: 1, points: points.length });
}
