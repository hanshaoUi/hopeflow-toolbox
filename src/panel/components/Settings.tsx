...
type Section = 'libraries' | 'online' | 'ai' | 'about' | 'donate';

const QrPlaceholder: React.FC<{ src: string; label: string }> = ({ src, label }) => (
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
    {src ? (
      <img src={src} alt={label} style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', borderRadius: 8, border: '1px solid var(--color-border)', display: 'block' }} />
    ) : (
      <div style={{ width: '100%', aspectRatio: '1', borderRadius: 8, border: '1px dashed var(--color-border)', background: 'var(--color-bg-primary)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, minHeight: 90 }}>
        <svg viewBox="0 0 48 48" width={36} height={36} fill="none" style={{ color: 'var(--color-text-tertiary)', opacity: 0.4 }}>
        ...
        <div style={{ fontSize: 9, color: 'var(--color-text-tertiary)', textAlign: 'center', lineHeight: 1.4 }}>替换为{label}图片</div>
      </div>
    )}
  </div>
);
...
export const Settings: React.FC = () => {
  ...
  const chooseLibraryFolder = () => {
    ...
    if (!fullPath || settings.libraries.some((e) => e.path === fullPath)) return;
  ...
  const commitRename = () => {
    ...
    if (nextName) update('libraries', settings.libraries.map((e, i) => i === renamingIndex ? { ...e, name: nextName } : e));
  ...
  const Accordion: React.FC<{ id: Section; icon: string; iconBg: string; title: string; desc: string; children: React.ReactNode }> = ({ id, icon, iconBg, title, desc, children }) => {
      ...
      <div style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {isOpen && (
          <div style={{ padding: '0 14px 14px' }}>
            <div style={{ height: 1, background: 'var(--color-border)', margin: '0 0 10px' }} />
            {children}
          </div>
        )}
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {settings.libraries.length === 0
        ...
        : settings.libraries.map((lib, i) => (
            <div key={lib.path + i} style={{ ...card, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {renamingIndex === i
                  ...
                  : <div onDoubleClick={() => { setRenamingIndex(i); setRenamingValue(lib.name); }} title="双击重命名"
                      style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-primary)', cursor: 'text' }}>{lib.name}</div>
                }
                <div style={{ ...label, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }} title={lib.path}>{lib.path}</div>
              </div>
              <button type="button" onClick={() => removeLibrary(i)} style={{ ...btn, minHeight: 26, padding: '0 8px', color: 'var(--color-error)', background: 'rgba(227,72,80,0.10)', border: 'none' }}>删除</button>
            </div>
          ))
      }
      <button type="button" onClick={chooseLibraryFolder} style={{ ...btn, background: 'var(--color-accent)', color: '#fff', border: 'none' }}>
        添加素材库文件夹
      </button>
    </div>
  );
  ...
  const aboutContent = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 0 6px' }}>
        <div style={{ width: 44, height: 44, borderRadius: 13, background: 'linear-gradient(135deg, #1aa3ff 0%, #0066cc 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', flexShrink: 0, boxShadow: '0 3px 12px rgba(26,163,255,0.30)' }}>
          <Icon name="sparkle" size={20} />
        </div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--color-text-primary)', letterSpacing: '-0.3px' }}>HopeFlow Toolbox</div>
          <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', marginTop: 2 }}>v{APP_VERSION} · Adobe Illustrator 插件</div>
        </div>
      </div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 30, height: 30, borderRadius: 9, background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a24 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', flexShrink: 0, fontSize: 13, fontWeight: 800 }}>H</div>
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-primary)' }}>hanshaoUi</div>
          <div style={{ ...label, marginTop: 1 }}>sayloveyouless@gmail.com</div>
        </div>
      </div>
      {[
        { icon: 'code', label: '问题反馈 / GitHub Issues', url: 'https://github.com/hanshaoUi/hopeflow-toolbox/issues' },
        { icon: 'file-text', label: '使用文档 / README', url: 'https://github.com/hanshaoUi/hopeflow-toolbox#readme' },
      ].map((item) => (
        <button key={item.url} type="button" onClick={() => openExternal(item.url)}
          style={{ ...card, width: '100%', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', textAlign: 'left', border: '1px solid var(--color-border)' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {[
        ...
        <div key={item.label} onClick={() => item.url && openExternal(item.url)}
          style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: item.url ? 'pointer' : 'default' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '2px 0 8px' }}>
      ...
      <div style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: 12, overflow: 'hidden' }}>
        <Accordion id="about" icon="info" iconBg="linear-gradient(135deg, #34c759 0%, #28a745 100%)" title="关于" desc={`HopeFlow v${APP_VERSION}`}>
          {aboutContent}
        </Accordion>
        <div style={{ height: 1, background: 'var(--color-border)' }} />
        <Accordion id="donate" icon="heart" iconBg="linear-gradient(135deg, #ff6b6b 0%, #e53935 100%)" title="赞赏开发者" desc="请我喝杯咖啡 ☕">
          {donateContent}
        </Accordion>
      </div>
    </div>
